...
run_patchtst.py
================
跑 PatchTST (Nie et al., ICLR 2023) baseline。

整体结构 1:1 对照 ``run_transformer.py`` / ``run_informer.py``，只改了 3 处：
    (1) 模型从 ``TransformerWrapper`` 换成 ``PatchTSTWrapper``
    (2) 默认超参换成 TSlib scripts/PatchTST/ETTh1.sh 标准
        (d_model=128, n_heads=4, e_layers=3, d_ff=256, patch=16, stride=8)
    (3) PatchTST 不用时间戳，wrapper 内部把 x_mark_enc / x_mark_dec 丢弃
        —— dataset 仍然返回 4 元组（公平对比要求），训练循环写法不变
...
pred_len = 4
lookback_len = 168
label_len = 48                              # wrapper 接口对齐用，PatchTST 内部不用
num_variates = 5                            # 5 列原始：GHR / Py / WS / TP1 / AP
target_idx = 4                              # AP 索引（在 5 列里仍然是第 5 个 → idx=4）

# ---------- PatchTST 模型超参（对齐 backbone 容量：d=128, d_ff=256, dropout=0.1） ----------
patch_d_model = 128
patch_n_heads = 4
patch_e_layers = 3
patch_d_ff = 256
...
# ========================== 训练与评估（4 输入接口，PatchTST 内部丢弃 mark） ==========================
...
def main():
    ...
    print(f"Lookback: {lookback_len}, Pred: {pred_len}, Patch: {patch_len}, Stride: {patch_stride}")
    ...
    config = {
        ...
        "patch_d_model": patch_d_model, "patch_n_heads": patch_n_heads,
        "patch_e_layers": patch_e_layers, "patch_d_ff": patch_d_ff,
        "patch_dropout": patch_dropout, "patch_activation": patch_activation,
