...
def build_model(spec):
    ...
    return AutoformerWrapper(
        num_variates=spec.num_variates,
        seq_len=spec.lookback_len,
        label_len=spec.label_len,
        pred_len=spec.pred_len,
        d_model=h["auto_d_model"],
        n_heads=h["auto_n_heads"],
        e_layers=h["auto_e_layers"],
        d_layers=h["auto_d_layers"],
        d_ff=h["auto_d_ff"],
        dropout=h["auto_dropout"],
        activation=h["auto_activation"],
        factor=h["auto_factor"],
        moving_avg=h["auto_moving_avg"],
        embed=h["auto_embed"],
        freq=h["auto_freq"],
    )
...
def get_run_config():
    return {
        ...
        "model_hparams": {
            "auto_d_model": auto_d_model,
            "auto_n_heads": auto_n_heads,
            "auto_e_layers": auto_e_layers,
            "auto_d_layers": auto_d_layers,
            "auto_d_ff": auto_d_ff,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Autoformer final protocol")
