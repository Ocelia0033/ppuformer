...
from layers.phys_attention import PhysBiasMLP
...
class Attention(Module):
    def __init__(
        ...
        attend_flash = flash and not use_phys_bias
        self.attend = Attend(flash = attend_flash, dropout = dropout)
        ...
        self.use_phys_bias = use_phys_bias
        self.num_mem_tokens = num_mem_tokens
        # 论文 2.4 节明确：PGIA 不引入额外标量门，PPU 通过 PhysBiasMLP 末层零初始化实现
        if use_phys_bias:
            assert num_variates is not None, 'use_phys_bias=True 时必须指定 num_variates'
            self.phys_bias = PhysBiasMLP(
        ...
        self._current_phi = None

    def set_phi(self, phi):
        self._current_phi = phi

    def _build_phys_attn_bias(self, q):
        """
        PGIA bias 构建（对应论文公式 20）
            scores_l = Q_l K_l^T / sqrt(d_k) + PhysBias_l(φ)

        论文 2.4 节："PGIA 不再额外引入标量门，而是通过将物理偏置网络的末层权重和
        偏置零初始化，使物理偏置在训练初始阶段为零。"——故此处直接返回 PhysBias，
        不再乘任何标量。
        """
        if not self.use_phys_bias or self._current_phi is None:
            return None

        phi = self._current_phi                              # [B, 4]
        n_tot = q.shape[-2]
        n_mem = self.num_mem_tokens
        N = self.phys_bias.num_variates

        bias_NN = self.phys_bias(phi)                        # [B, N, N]
        bias_full = q.new_zeros(phi.shape[0], n_tot, n_tot)
        bias_full[:, n_mem:n_mem + N, n_mem:n_mem + N] = bias_NN
        return bias_full.unsqueeze(1)
    ...
    def forward(
        ...
        attn_bias = self._build_phys_attn_bias(q)
...
class iTransformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        pgia_num_variates = num_variates * num_tokens_per_variate
        ...
        for i in range(depth):
            ...
            self.layers.append(ModuleList([
                init_hyper_conn(dim = dim, branch = Attention(
                    ...
                    use_phys_bias = use_pgia,
                    num_variates = pgia_num_variates,
    ...
    def forward(
        ...
        if exists(phi):
            for module in self.modules():
                if isinstance(module, Attention) and getattr(module, 'use_phys_bias', False):
                    module.set_phi(phi)
