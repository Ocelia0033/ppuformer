...
整体结构 1:1 对照 ``run_patchtst.py``（同样不需要时间戳的 baseline），只改了 3 处：
    (1) 模型从 ``PatchTSTWrapper`` 换成 ``LSTMWrapper``
    (2) 默认超参换成 PV 预测论文常见配置 (hidden=128, num_layers=2, dropout=0.1)
    (3) config 字典里超参名前缀换成 ``lstm_*``
...
pred_len = 4
lookback_len = 168
label_len = 48                              # wrapper 接口对齐用，LSTM 内部不用
...
# ---------- 训练超参（与 iTransformer / 其它 baseline 完全一致：裸 Adam，无调度器、无梯度裁剪） ----------
epochs = 100
batch_size = 128
learning_rate = 0.00038705458701655343
seed = 35040
train_ratio = 0.8
...
def main():
    ...
    print(f"Lookback: {lookback_len}, Pred: {pred_len}, Target idx: {target_idx}")
    print(f"LSTM: hidden={lstm_hidden_size}, layers={lstm_num_layers}, "
          f"dropout={lstm_dropout}, bidirectional={lstm_bidirectional}")
