...
def build_model(spec):
    ...
    return FEDformerWrapper(
        num_variates=spec.num_variates,
        seq_len=spec.lookback_len,
        label_len=spec.label_len,
        pred_len=spec.pred_len,
        d_model=h["fed_d_model"],
        n_heads=h["fed_n_heads"],
        e_layers=h["fed_e_layers"],
        d_layers=h["fed_d_layers"],
        d_ff=h["fed_d_ff"],
        dropout=h["fed_dropout"],
        activation=h["fed_activation"],
        moving_avg=h["fed_moving_avg"],
        embed=h["fed_embed"],
        freq=h["fed_freq"],
        version=h["fed_version"],
        mode_select=h["fed_mode_select"],
        modes=h["fed_modes"],
    )
...
def get_run_config():
    return {
        ...
        "model_hparams": {
            "fed_d_model": fed_d_model,
            "fed_n_heads": fed_n_heads,
            "fed_e_layers": fed_e_layers,
            "fed_d_layers": fed_d_layers,
            "fed_d_ff": fed_d_ff,
            "fed_dropout": fed_dropout,
            "fed_activation": fed_activation,
            "fed_moving_avg": fed_moving_avg,
            "fed_embed": fed_embed,
            "fed_freq": fed_freq,
            "fed_version": fed_version,
            "fed_mode_select": fed_mode_select,
            "fed_modes": fed_modes,
        },
    }
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run FEDformer final protocol")
